...
internal class Security
{
	...
	public static void Start()
	{
		...
		{
			MessageBox.Show("There's another session linked to your account, close it first please!", OnProgramStart.Name);
		...
		using (StreamReader streamReader = new StreamReader(pathRoot + "Windows\\System32\\drivers\\etc\\hosts"))
		{
			if (streamReader.ReadToEnd().Contains("api.auth.gg"))
			{
				...
				MessageBox.Show("It seems that DNS are being redirected; check hosts file (GA L 799)", OnProgramStart.Name);
				Process.GetCurrentProcess().Kill();
			}
		}
	...
	public static void End()
	{
		if (!Constants.Started)
		{
			MessageBox.Show("No session was started, now closing for safety reasons! (GA L 818)", OnProgramStart.Name);
			Process.GetCurrentProcess().Kill();
			return;
		}
